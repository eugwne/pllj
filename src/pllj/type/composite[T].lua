...
local function composite_unbox(self)
    ...
    local field_info = rtti[4][2]
    local field_values = composite[1]
    local size = rtti[4][3]
    local t = table_new(0, size)
    local t2 = table_new(0, size)
    for i = 1, size do
        local attname = field_info[i][1]
        local atttypid = field_info[i][2]
...
composite_to_datum = function (self)
    ...
    if updated then
        ...
        rawset(self, -1, C.HeapTupleHeaderGetDatum(
                    C.SPI_copytuple(lua_table_to_tuple_2(tuple_desc, t2, field_info)).t_data
                ))
    end
...
local mm__composite_newindex = function(self, key, value)
